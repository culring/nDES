...
def test_poor_selective_matmul(setup_data):
    ...
    result_cuda = poor_selective_matmul(matrix, sorting_idx.argsort().int(), vec, result_cuda, 2000)
...
def test_selective_weights_matmul(setup_data):
    ...
    sorted_weights = create_sorted_weights_for_matmul(vec, sorting_idx, sorted_weights, 2000)

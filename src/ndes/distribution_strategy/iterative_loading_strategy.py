...
class IterativeLoadingStrategy(DistributionStrategy):
    ...
    def evaluate(self, population):
        ...
        total_batch_indices = []
        individuals_left = population_size

        while individuals_left > self.max_individuals_per_node * self.num_nodes:
        ...
        if individuals_left > 0:
            num_individuals_per_node = individuals_left // self.num_nodes
            remaining_individuals_num = individuals_left - num_individuals_per_node * self.num_nodes
            for node in self.nodes:
                num_individuals = num_individuals_per_node
                if remaining_individuals_num > 0:
                    remaining_individuals_num -= 1
                    num_individuals += 1
